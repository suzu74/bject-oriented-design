...
* 組み合わされた全体が、単なる部品の集合以上となるように、個別の部品を複雑な全体へと組み合わせること
* シンプルで独立したオブジェクトを、より大きく、より複雑な全体へと組み合わせる
    * より大きいオブジェクトと部品が`has_a`
...
* 自転車がどのようにコンポーズされる考える
* 自転車はパーツを持ち、自転車ー部品といった関係になる
* `Parts`クラスが自転車のパーツ一覧を保持する（partではなくparts　つまり全体）

* 自転車であるということは部品をもつということ
    * 全てのBicycleがpartsオブジェクトを必要としている
...
* パーツリストは個々の部品をもつため、単一の部品を表すクラスを追加

##### Partをつくる

* partsオブジェクトをつくる必要が出てくる
    * Partsは複数のPartオブジェクトからコンポーズされる

##### Partsオブジェクトをもっと配列のように

* `parts`と`spares`はどちらも配列のようだが実際は違う
* 複雑さと利便性の中間の解決策の一つに`Enumerable`をインクルードする

#### 8.3 Partsを製造する

* 個別のパーツは多岐にわたり、いくつもあるが、有効なパーツの組み合わせはほんの一部
    * 色々な自転車を記述し、その記述をもとに正確なPartsオブジェクトをどの自転車にも製造できれば良さそう

##### PartsFactoryをつくる

* 他のオブジェクトを製造するオブジェクトはファクトリー
    * 他のオブジェクトとをつくるオブジェクト

##### PartsFactoryを活用する

* `OpenStruct`はいくつもの属性を1つのオブジェクトにまとめるための便利な方法を提供してくれる

#### 8.4 コンポーズされたBicycle

* BicycleとPartsはhas-aの関係
    * PartsもPartとhas-aの関係になる

#### 8.5 コンポジションと継承の選択

* 一般的にコンポジションが依存が少ないので、継承が絶対に良いと言えない場合はコンポジションにする
...
* 継承のコストと利点について
    * 利点
        * 正しくモデル化された階層構図は合理的
        * 利便性が高い
        * 既存のパターンを模倣できるため、階層構造は模範的
    * コスト
        * 継承が適さない問題に対して継承を選択すること
        * 正しい継承でも他のプログラマーによって全く予期していない目的に使われる可能性があること

##### コンポジションによる影響を認める

* コンポジションによって作られたオブジェクトは階層構造に依存しない
* 自身のメッセージは自身で委譲する
    
* コンポジションのコストと利点について
    * 利点
        * 責任が明快で、明確に定義されたインターフェースを介してアクセス可能の小さなオブジェクト
        * 単一責任をもち、自身の振る舞いを限定
        * 階層構造のような上の階層の変化による影響がない
        * 新しい部品の追加は、単純にそのインターフェースを守る新しいオブジェクトを差し込むだけ
    * コスト
        * コンポーズされたオブジェクトは多くのパーツに依存する
        * 部品の動作は明快でも、全体の動作は理解しやすいとは限らない

##### 関係の選択

* 継承やコンポジションの技術を適切に使うことが大事
* is-a関係には継承
* behaves-like-aの関係にダックタイピング
* has-aの関係にコンポジション

#### 8.6 まとめ

* コンポジション、継承、モジュールはそれぞれに利点とコストがある
    * 適切に使いわけることが大事
...
* 多くの人はテストを書きすぎ
* テストからより良い価値を得るためには、重複を無くしより少なくテストを書くこと
* 受診するか送信するメッセージに集中すべきである
    * 受診メッセージはその戻り値の状態をテスト
    * 送信は送られたことがテストされるべき
...
* 受診メッセージはオブジェクトのパブリックインターフェースを構成する
* 他のオブジェクトがその戻り値などに依存しているためテストの必要がある

##### 使われていないインターフェースを削除する

* 依存されていない受診メッセージはテストしない
* 活発の使われていないコードを削除することでアプリは改善されていく

##### パブリックインターフェースを証明する

* 受診メッセージは実行によって戻される値や状態をテストする

##### テスト対象のオブジェクトを隔離する

* あるオブジェクトが独立してテストできない
    * つまりそれは特定のコンテキストに結び付けられている

##### クラスを使って依存オブジェクトを注入する

* テストでもアプリでも、同じオブジェクトと協力していれば、テストは壊れるべき時に壊れる

##### ロールとして依存オブジェクトを注入する

* アプリがコードがただ1つの方法でしか書かれないのなら、その構成をそのまま反映するのが最も効果的にテストを書く方法である場合もある
* しかし、いくつも同じ役割をもつものがあれば1つ理想のものを1つ作るのも良い
* そのロールを担うフェイクオブジェクトを作るのも1つの案
    * テストだけで使われるテストダブルも1つ
        * スタブやダブルはアプリを脆くするという考えもある

#### 9.3 プライベートメソッドをテストする

* テスト対象は自分自身にメッセージを送ることもある
...
* プライベートメソッドはテストの必要が基本的にない

#### 9.4 送信メッセージをテストする

* 送信メッセージはクエリかコマンドのどちらか
